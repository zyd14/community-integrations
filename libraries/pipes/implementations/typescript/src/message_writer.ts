...
export class PipesDefaultMessageWriter extends PipesMessageWriter {
    ...
    if (messagesParams['path']) {
      ...
      if (messagesParams[INCLUDE_STDIO_IN_MESSAGES_KEY]) {
        throw new DagsterPipesError(
          'include_stdio_in_messages is not yet implemented in the typescript-pipes framework',
        );
      }
      ...
      if (messagesParams[STDIO] == STDOUT) {
        ...
        throw new DagsterPipesError(
          `valud for 'stdio' key must be either 'stdout' or 'stderr'. Receeived ${messagesParams['stdio']}`,
        );
      }
      ...
      if (messagesParams[BUFFERED_STDIO] == STDOUT) {
        ...
        throw new DagsterPipesError(
          `value for 'buffered_stdio' key must be either 'stdout' or 'stderr'. Received ${messagesParams['buffered_stdio']}`,
        );
      }
    } else {
      throw new DagsterPipesError(
        "Either 'path', 'stdio', or 'buffered_stdio' must be provided in messagesParams",
      );
    }
  }
  ...
  public writeMessage(message: PipesMessage): void {
    if (!this.isOpen) {
      throw new DagsterPipesError(
        'MessageWriter.writeMessage cannot be called before MessageWriter.open()',
      );
    }
    if (this.isClosed) {
      throw new DagsterPipesError(
        'MessageWriter.writeMessage cannot be called after MessageWriter.close()',
      );
    }
