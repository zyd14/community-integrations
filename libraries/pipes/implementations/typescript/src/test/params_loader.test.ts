...
describe('PipesEnvParamsLoader', () => {
  ...
  test('loads context params from environment variable', () => {
    ...
    const encodedContextData = zlib
      .deflateSync(Buffer.from(JSON.stringify(contextData)))
      .toString('base64');
  ...
  test('loads message params from environment variable', () => {
    ...
    const encodedMessageData = zlib
      .deflateSync(Buffer.from(JSON.stringify(messageData)))
      .toString('base64');
